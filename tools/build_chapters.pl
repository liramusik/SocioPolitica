...
{
    my $glob_path = catfile( 'Documentos', "$doc",'src', 'chapter_??.pod' );
...
{
    my %sections;
    my $sections_path = catfile( 'Documentos', "$doc",'src', '*.pod' );
...
sub write_chapter
{
    ...
    my $chapter_dir   = catdir( 'Documentos', "$doc",'build','chapters' );
